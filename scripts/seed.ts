 ...
 * (Jan–Jun 2026, spanning FY 2025-26 → FY 2026-27).
 * Run: npm run seed
 */
import { createProperty, listProperties, upsertPeriod } from '../src/lib/db';
import { emptyPeriodData } from '../src/lib/engine';
import { daysInPeriod } from '../src/lib/fiscal';

const NAME = 'Tamarind Court Hotel';

if (listProperties().some(p => p.name === NAME)) {
  console.log(`"${NAME}" already exists — nothing to do. Delete it in Property Settings to re-seed.`);
  process.exit(0);
}

const prop = createProperty({ name: NAME, city: 'Jaipur', rooms: 42, ccy: 'INR' });

// Rajasthan seasonality: strong winter, soft summer.
...
for (const m of MONTHS) {
  ...
  console.log(`Seeded ${m.period}: ${sold} room-nights, ADR ₹${m.adr}`);
}

console.log(`\nDone — "${NAME}" (id ${prop.id}) with ${MONTHS.length} months of data.`);
